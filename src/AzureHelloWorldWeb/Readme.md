...
Dockerfile has a **COPY** command which copies the value from the directory pointed to
by `--docker-host-build-output-dir` to the `/var/task` directory inside of the image.
...
built inside the container. Below is an example of building .NET 5 project inside the image.
